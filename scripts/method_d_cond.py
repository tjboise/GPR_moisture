"""
Method D — Condition-aware: STFT + 2D CNN with condition label injected into FC head
Architecture change: after CNN encoder, concatenate one-hot(condition) before Linear layers.
This tells the model explicitly which soil/pipe type it is working with.
...
print(f'STFT image: {H}×{W}  |  Condition-aware head: concat one-hot({N_COND}) before FC')

# Pre-compute all STFT images
imgs = np.stack([ascan_to_stft(X_norm[i]) for i in range(len(X_norm))])  # (135, H, W)


# ── Dataset ────────────────────────────────────────────────────────────────
class GPRDataset(Dataset):
    def __init__(self, imgs, cond_arr, y_norm):
        self.imgs = torch.tensor(imgs[:, None, :, :], dtype=torch.float32)
        self.cond = torch.tensor(cond_arr, dtype=torch.long)
        self.y    = torch.tensor(y_norm, dtype=torch.float32)

    def __len__(self): return len(self.imgs)

    def __getitem__(self, i):
        return self.imgs[i], self.cond[i], self.y[i]


# ── Model ──────────────────────────────────────────────────────────────────
class CNN2DCond(nn.Module):
    """Same encoder as baseline; FC head receives CNN features + condition one-hot."""
    def __init__(self, n_cond=N_COND):
        super().__init__()
        ...
        feat_dim = 64 * 4 * 4 + n_cond  # 1024 + 3 = 1027
    ...
    def forward(self, x, c):
        ...
        c_oh   = F.one_hot(c, num_classes=N_COND).float()        # (N, 3)
        return self.head(torch.cat([feat, c_oh], dim=1))


def train_fold(imgs_tr, cond_tr, y_tr,
               imgs_te, cond_te, y_te, fold_idx):
    ...
    tr_ds = GPRDataset(imgs_tr, cond_tr, y_tr_n)
    te_ds = GPRDataset(imgs_te, cond_te, y_te_n)
    tr_dl = DataLoader(tr_ds, batch_size=16, shuffle=True)

    # Build full validation tensors once
    X_te_t  = torch.stack([te_ds[i][0] for i in range(len(te_ds))])
    c_te_t  = torch.tensor(cond_te, dtype=torch.long)
    ...
    for epoch in range(800):
        ...
        for xb, cb, yb in tr_dl:
            opt.zero_grad(); crit(model(xb, cb), yb).backward(); opt.step()

        model.eval()
        with torch.no_grad():
            val_loss = crit(model(X_te_t, c_te_t), y_te_t).item()
    ...
    model.eval()
    with torch.no_grad():
        y_pred_n = model(X_te_t, c_te_t).numpy()
...
for fold_i, (tr_idx, te_idx) in enumerate(skf.split(imgs, cond)):
    ...
    y_pred = train_fold(
        imgs[tr_idx], cond[tr_idx], y[tr_idx],
        imgs[te_idx], cond[te_idx], y[te_idx],
        fold_i
    )
...
fig.suptitle('STFT + 2D CNN (condition-aware head) — 5-fold CV, n=135',
...
plt.tight_layout()
plt.savefig('../results/fig_methodD_cond_scatter.png', dpi=150, bbox_inches='tight')
plt.close()
print('\nSaved: fig_methodD_cond_scatter.png')
